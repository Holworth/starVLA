...
class _QWen3_5_VL_Interface(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        if "labels" not in kwargs and "logits_to_keep" not in kwargs:
            # [OPT #4] Training via QwenPI only consumes hidden_states;
            # computing the full 248320-vocab lm_head projection wastes a
            # multi-ms GEMM and a ~700MB bf16 logits buffer per step. Keep one
            # position so the output shape stays valid for any downstream
            # shape checks.
