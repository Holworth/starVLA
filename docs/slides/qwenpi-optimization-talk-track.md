...
**讲稿**:最大单项。左图:基线 37% 的 GPU 计算是 fp32 GEMM,全部来自 2.96B 的动作头。修复是一个开关:DiT 主体进 bf16,timestep 编码与 loss 数学保持 fp32。右图正确性:1000 步、仅 dtype 不同的对照,末段 100 步均值两边都是 0.1473。

- **Q: loss 一致 ≠ 任务成功率一致?** A: 同意——任务级评测要在你们的评测器上做,这是建议的联合验证项。我们的证据:千步损失轨迹统计不可区分 + 数值敏感处保留 fp32,与业界 VLA 训练通行做法一致。
- **Q: 为什么当初是 fp32?** A: 参考实现对 diffusion 数值的保守继承。逐步 |Δ|=0.02 全部来自未固定种子的扩散采样;需要的话可加固定采样种子做逐位验证。
